...
async function callGemini(
  ...
  try {
    // Build Gemini-formatted conversation contents
    // Gemini uses "user" and "model" roles
    const contents = messages.slice(-8).map(m => ({
      role: m.role === "assistant" || m.role === "echo" ? "model" : "user",
    ...
    if (lastUserIndex !== -1) {
      const idx = contents.length - 1 - lastUserIndex;
      contents[idx].parts.push({
        text: `\n\n[Available patient context — use only this, never invent data]:\n${JSON.stringify(contextSummary, null, 2)}`,
      });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Distress check — always takes priority
    ...
    // Try Gemini first
    const geminiReply = await callGemini(messages as ChatMessage[], summary, systemPrompt);
    if (geminiReply && !violatesMedicalBoundary(geminiReply)) {
      return NextResponse.json({ message: geminiReply });
    }

    // Fallback to rule-based reply if Gemini is unavailable or violates boundary
