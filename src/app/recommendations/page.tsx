...
const defaultLifestyle: RecommendationItem[] = [
...
const defaultDiet: RecommendationItem[] = [
...
const defaultExercise: RecommendationItem[] = [
...
const defaultSleep: RecommendationItem[] = [
...
const defaultStress: RecommendationItem[] = [
...
const defaultDoctorRecs: DoctorRecItem[] = [
  {
    dept: "Rheumatology",
...
export default function RecommendationsPage() {
  ...
  // Dynamic recommendations states
  const [lifestyle, setLifestyle] = useState<RecommendationItem[]>(defaultLifestyle);
  const [diet, setDiet] = useState<RecommendationItem[]>(defaultDiet);
  const [exercise, setExercise] = useState<RecommendationItem[]>(defaultExercise);
  const [sleep, setSleep] = useState<RecommendationItem[]>(defaultSleep);
  const [stress, setStress] = useState<RecommendationItem[]>(defaultStress);
  const [doctorRecs, setDoctorRecs] = useState<DoctorRecItem[]>(defaultDoctorRecs);

  useEffect(() => {
    if (typeof window !== "undefined") {
      const surveyStr = localStorage.getItem("echocare-survey");
      const analysisStr = localStorage.getItem("echocare-story-analysis");

      if (surveyStr) {
        try {
          const survey = JSON.parse(surveyStr);
          const symptoms = Array.isArray(survey.symptoms) ? survey.symptoms : [];
          
          // ── Lifestyle Averages/Personalization ──────────────────────────────
          const nextLifestyle = [...defaultLifestyle];
          if (survey.dailyRoutine?.toLowerCase().includes("sit") || survey.dailyRoutine?.toLowerCase().includes("desk")) {
            nextLifestyle.unshift({ title: "Incorporate active desk breaks: stand up and stretch every 60 minutes", priority: "High", tag: "Routine", done: false });
          }
          if (symptoms.includes("Fatigue")) {
            nextLifestyle.unshift({ title: "Pace your activities: distribute tasks to conserve baseline energy", priority: "High", tag: "Energy", done: false });
          }
          setLifestyle(nextLifestyle);

          // ── Diet Personalization ────────────────────────────
          const nextDiet = [...defaultDiet];
          if (symptoms.includes("Digestive Issues") || symptoms.includes("Nausea")) {
            nextDiet.unshift({ title: "Eat smaller, warm, easily digestible meals every 3-4 hours", priority: "High", tag: "Digestion", done: false });
            nextDiet.unshift({ title: "Eliminate carbonated, highly acidic, and overly spicy food", priority: "High", tag: "Digestion", done: false });
          }
          setDiet(nextDiet);

          // ── Exercise Personalization ────────────────────────────
          const nextExercise = [...defaultExercise];
          if (symptoms.includes("Joint Pain") || symptoms.includes("Muscle Weakness")) {
            nextExercise.unshift({ title: "Perform gentle, low-impact joint mobility stretches twice daily", priority: "High", tag: "Joints", done: false });
          }
          setExercise(nextExercise);

          // ── Sleep Personalization ────────────────────────────
          const nextSleep = [...defaultSleep];
          if (survey.sleepQuality?.toLowerCase().includes("poor") || survey.sleepQuality?.toLowerCase().includes("restless")) {
            nextSleep.unshift({ title: "Set a screen-free wind-down routine starting 45 mins before bedtime", priority: "High", tag: "Sleep Hygiene", done: false });
          }
          setSleep(nextSleep);

          // ── Stress Personalization ────────────────────────────
          const nextStress = [...defaultStress];
          if (survey.stressLevel?.toLowerCase() === "high" || survey.stressLevel?.toLowerCase() === "extreme") {
            nextStress.unshift({ title: "Engage in a 5-minute deep diaphragmatic breathing exercise at midday", priority: "High", tag: "Stress Recovery", done: false });
          }
      ...
      if (analysisStr) {
        try {
          ...
          if (Array.isArray(analysis.suggestedDepartments) && analysis.suggestedDepartments.length > 0) {
            const mapped = analysis.suggestedDepartments.map((d: any) => {
              let icon = "🏥";
              const deptLower = d.dept.toLowerCase();
              if (deptLower.includes("rheumatology")) icon = "🦴";
              else if (deptLower.includes("endocrinology")) icon = "⚗️";
              else if (deptLower.includes("sleep")) icon = "🌙";
              else if (deptLower.includes("cardiology")) icon = "❤️";
              else if (deptLower.includes("neurology")) icon = "🧠";
              else if (deptLower.includes("gastroenterology")) icon = "🥦";
              
              return {
                dept: d.dept,
                reason: d.reason || "Correlation flagged in story analysis.",
                confidence: d.confidence || 75,
                urgency: d.confidence >= 85 ? "Medium" : "Low",
                icon
              };
            });
            setDoctorRecs(mapped);
          }
        } catch (_) {}
      }
    }
  ...
  // Dynamic selector for active recommendations array
  const getActiveRecs = () => {
    switch (activeTab) {
            ...
            {recs.map((rec, i) => {
              ...
              return (
                <div key={i} className="card" style={{
                  ...
                  <button className="btn btn-ghost btn-sm" style={{ color: "#0F766E", fontWeight: 600 }} onClick={() => alert(`Recommendation Details:\n\n${rec.title}\n\nPriority: ${rec.priority}\nCategory: ${rec.tag}`)}>
            ...
            {doctorRecs.map((dr, i) => (
              <div key={i} className="card" style={{ padding: "24px" }}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <button className="btn btn-primary btn-sm" onClick={() => alert(`Finding a ${dr.dept} specialist near you...`)}>Find a {dr.dept} Specialist</button>
                  <button className="btn btn-secondary btn-sm" onClick={() => alert(`Redirecting to book a consultation for ${dr.dept}...`)}>Book Consultation</button>
                </div>
              </div>
            ))}
          </div>
