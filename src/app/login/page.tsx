...
import { loginWithBackend, fetchCurrentUser, BACKEND_URL } from "@/lib/backend";
...
export default function LoginPage() {
  ...
  const handleGoogleLoginSuccess = async (response: any) => {
    ...
    try {
      ...
      const hasSurvey = typeof window !== "undefined" && !!localStorage.getItem("echocare-survey");
      login({
        id: dbUser.id,
      ...
      router.push(hasSurvey ? "/dashboard" : "/survey");
    } catch (err: any) {
  ...
  const initGoogle = () => {
    ...
    if (google) {
      google.accounts.id.initialize({
        client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "543477464295-p789kdf5nve8tr3kef1a4p120895o9e8.apps.googleusercontent.com",
  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      login({
        ...
        email: dbUser.email,
        activePlan: dbUser.active_plan,
        surveyCompleted: hasSurvey, // will be updated by survey page
        token: access_token,
      });
